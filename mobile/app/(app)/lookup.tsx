/**
 * ParkiPay — Vehicle Lookup Screen  (v3)
 *
 * Uses system keyboard (no custom keypad).
 * Plate input: capital letters only, auto-formatted T 000 AAA.
 * Result cards shown below the input, not in a bottom sheet.
 * Modals used for duplicate bill warning and bill-generated success.
 */
...
import { pushBillSuccess, pushDuplicateAlert, pushBillFailed } from '@/services/alertsService';
...
interface VehicleInfo {
  id: number; plateNumber: string; ownerName: string; ownerPhone: string;
  make?: string; model?: string; category?: string; isValid?: boolean;
}
...
type LookupState = 'idle' | 'loading' | 'found' | 'not_found' | 'duplicate' | 'generating' | 'success';
...
export default function LookupScreen() {
  ...
  const [genBill,   setGenBill]   = useState<{ controlNumber: string; amountDue: number } | null>(null);
  ...
  const normalise = (raw: string) => raw.trim().toUpperCase().replace(/\s/g, '');

  const handleVerify = async () => {
    Keyboard.dismiss();
    const plate = normalise(plateRaw);
    if (plate.length < 4) { shake(); return; }

    setLookupState('loading');
    setVehicle(null); setActiveBill(null);
    ...
    try {
      ...
      const foundVehicle = vRes.status === 'fulfilled' ? vRes.value.data : null;
  ...
  const handleGenerateBill = useCallback(async () => {
    ...
    try {
      ...
      const bill = res.data;
      setGenBill({ controlNumber: bill.controlNumber, amountDue: Number(bill.amountDue) });
      setLookupState('success');
      setShowSuccessModal(true);
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      // Push alert for Alerts screen
      pushBillSuccess({
        plateNumber:   plate,
        controlNumber: bill.controlNumber,
        location:      bill.location?.name ?? officer?.locationName ?? 'Unknown',
        issuedAt:      bill.generatedAt ?? new Date().toISOString(),
        expiresAt:     bill.expiresAt   ?? '',
      });
    } catch (e: any) {
      if (e?.response?.status === 409) {
        setActiveBill(e.response.data.existing_bill);
        setLookupState('duplicate');
        setShowDupModal(true);
      } else {
        const reason = e?.response?.data?.detail ?? 'Unknown error';
        pushBillFailed({ plateNumber: plate, reason });
        setLookupState(vehicle ? 'found' : 'not_found');
      }
    }
  ...
  const reset = () => {
    setCnCopied(false);
    setCnCopied(false);
    setPlateRaw(''); setVehicle(null); setActiveBill(null); setGenBill(null);
    setLookupState('idle');
  ...
  const showResult = lookupState === 'found' || lookupState === 'not_found';
  const plate = normalise(plateRaw);
      ...
      <ScrollView contentContainerStyle={S.body} keyboardShouldPersistTaps="handled"
        ...
        <Animated.View style={[S.inputRow, { transform: [{ translateX: shakeAnim }] }]}>
          ...
          <TextInput
            ...
            onChangeText={t => {
              // Only keep alphanumeric, uppercase
              const clean = t.replace(/[^A-Z0-9]/gi, '').toUpperCase().slice(0, 9);
              setPlateRaw(clean);
        ...
        {/* ── Verify Button ──────────────────────────────────────────── */}
        <TouchableOpacity style={[S.verifyBtn, (isLoading || plate.length < 4) && { opacity: 0.6 }]}
        ...
        {/* ── Camera hint ──────────────────────────────────────────────── */}
        <TouchableOpacity style={S.cameraHint}
          onPress={() => {}}>
        ...
        {/* ═══════════════════════════════════════════════════════════════
            RESULT AREA — shown after lookup (no more custom keyboard here)
        ════════════════════════════════════════════════════════════════ */}
        ...
        {lookupState === 'found' && vehicle && (
          <View style={[S.resultCard, { backgroundColor: C.card, borderColor: SprintColors.green }]}>
            ...
            {[
              ...
              { icon: 'tag-outline',     label: 'Category', val: vehicle.category?.replace(/_/g,' ') ?? 'Private Car' },
            ].map(r => (
          ...
          <View style={[S.resultCard, { backgroundColor: C.card, borderColor: '#3B82F6' }]}>
            <View style={S.resultHeaderRow}>
              <View style={[S.resultIconBg, { backgroundColor: 'rgba(59,130,246,0.1)' }]}>
                <MaterialCommunityIcons name="car-question" size={22} color="#3B82F6" />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[S.resultTitle, { color: C.text }]}>Not in Registry</Text>
                <Text style={[S.resultSub, { color: C.textSub }]}>
                  Plate <Text style={{ fontWeight: '800' }}>{formatPlate(plateRaw)}</Text> is unregistered. Bill can still be issued.
                </Text>
              </View>
            </View>
        ...
        <View style={[S.modalSheet, { backgroundColor: C.card }]}>
          ...
          <View style={[S.dupCard, { borderColor: SprintColors.yellow }]}>
            ...
            {[
              ...
              { label: 'Issuing Officer', value: `${activeBill?.issued_by ?? 'Unknown'} ${activeBill?.officer_id ? `(${activeBill.officer_id})` : ''}` },
              { label: 'Location',        value: activeBill?.location ?? '—' },
              { label: 'Amount Due',      value: activeBill?.amount_due
          ...
          <View style={[S.infoNote, { backgroundColor: 'rgba(252,209,22,0.08)', borderLeftColor: SprintColors.yellow }]}>
            ...
            <View style={[S.cnBox, { backgroundColor: C.bg }]}>
              ...
              <View style={S.cnRow}>
                ...
                <TouchableOpacity style={[S.copyBtn,
                  ...
                  <Ionicons name={cnCopied ? 'checkmark' : 'copy-outline'}
                    size={18} color={cnCopied ? SprintColors.green : C.textMuted} />
                </TouchableOpacity>
              </View>
...
function makeStyles(C: ReturnType<typeof palette>) {
  return StyleSheet.create({
    ...
    dupIconWrap: { width: 60, height: 60, borderRadius: 18,
      backgroundColor: 'rgba(252,209,22,0.15)', alignItems: 'center',
    ...
    liveBadge: { flexDirection: 'row', alignItems: 'center', gap: 5,
      backgroundColor: '#C9A800', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
    liveDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: '#fff' },
    liveBadgeText: { color: '#fff', fontSize: 10, fontWeight: '800' },
    dupRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 7,
      borderBottomWidth: 1 },
    ...
    cnLabel: { fontSize: 10, fontWeight: '700', letterSpacing: 2, textTransform: 'uppercase',
      marginBottom: 6 },
    ...
    doneBtn: { width: '100%', height: 50, borderRadius: 12,
      backgroundColor: SprintColors.green, alignItems: 'center',
