 ...
 * POST /api/vehicles/ocr-plate/       OCR plate extraction from base64 image
 */
...
router.post('/ocr-plate/', async (req, res, next) => {
  try {
    ...
    return res.json({
      ...
      rawText: rawText.trim(),
      detail:
        'Could not extract a valid Tanzania plate number from the image. Try again with better lighting.',
    });
  } catch (err) {
